import { currentTarget, currentTargetText } from "utils/participantTargets";
...
export function resolveStability(s) {
...
export function stabilityNumbers(eq) {
  if (!eq) return null;
...
export function ValidationReadout({ validation, validating, emitContext, onCommitted }) {
  ...
  const verdict = /VALIDATED \(stim-stable\)/.test(rawVerdict) && stability.key !== "stable"
    ? rawVerdict.replace("(stim-stable)",
        stability.key === "inconclusive" ? "(stim stability not determinable)"
          : stability.key === "stim-dependent" ? "(stim-dependent)" : "(stim stability not tested)")
    : rawVerdict;
            ...
            onClick={() => {
              ...
              queryAnalysis("/api/emitBandCandidate", {
                ...
                if (data && data.available && data.band_candidate) {
                  commitBandCandidate(emitContext.participantUid, data.band_candidate, { InputManifest: data.InputManifest, requestParams: emitContext.requestParams });
                  downloadBandCandidate(emitContext.participantUid, data.band_candidate, { InputManifest: data.InputManifest, requestParams: emitContext.requestParams });
                  if (typeof onCommitted === "function") onCommitted(data.band_candidate);
                  setCommitted({ ok: true, msg: "Selected for Closed-Loop Research Review (JSON downloaded)." });
                } else {
...
function SpectralFeatureImportance({ scan, pain, HI, LO, participantUid, requestParams, matchDirty,
  ...
  useEffect(() => {
    ...
    setValidating(true);
    setValidation(null);
    queryAnalysis("/api/queryBandValidation", {
    ...
    return () => { cancelled = true; queryAnalysis.cancel("/api/queryBandValidation"); };
  ...
  useEffect(() => {
    ...
    orderedChannels.forEach(({ ch, ci }) => {
      ...
      traces.push({ x: centers, y: ch.r, name: `${currentTargetText(ch.short)}${nch} · r`, type: "scattergl", mode: "lines",
      ...
      traces.push({ x: centers,
        ...
        name: `${currentTargetText(ch.short)}${nch} · AUC (signed)`, type: "scattergl", mode: "lines",
      ...
      if (fdrCenters.length) {
        traces.push({ x: fdrCenters, y: fdrRs, name: `${currentTargetText(ch.short)} · FDR`, type: "scattergl", mode: "markers",
    ...
    if (sc && sc.x && sc.x.length) {
        ...
        <MDBox mt={1}>
          ...
          <MDBox sx={{ textAlign: "center", mb: 1, mt: 2, pt: 1.5 }}>
            ...
            <MDTypography fontWeight="bold" color="dark"
              ...
              {`${currentTargetText(ch.short)} (n=${nShown}${srcLbl ? `: ${srcLbl}` : ""}${subsampled ? ` of ${sc.n_distinct}` : ""}) @ ${center.toFixed(1)} Hz`}
      ...
      scatterNode = (
        <MDTypography variant="caption" color="text" mt={1} display="block">
          {`No scatter for ${currentTargetText(ch.short)} @ ${center.toFixed(1)} Hz (fewer than 3 matched samples in this band).`}
        </MDTypography>
      );
    }
  }
    ...
    <Grid item xs={12}>
      <Card sx={{ width: "100%", scrollMarginTop: "96px" }}>
        <MDBox p={2}>
          ...
          {(() => {
            // Every channel the scan touched is listed, including the ones for which no band could
            // be selected. `selected_band` is null when not one band in the sweep produced a
            // computable correlation, which means the channel was ANALYSED AND YIELDED NOTHING —
            // a different statement from the channel not having been analysed. Filtering those
            // rows out, as this table used to, made the two indistinguishable: a reader comparing
            // a four-contact montage against a three-row table had no way to learn that the fourth
            // contact had been screened and had come back empty.
            const all = (scan && Array.isArray(scan.channels)) ? scan.channels : [];
            const rows = all.filter((c) => c.selected_band);
            const barren = all.filter((c) => !c.selected_band);
            if (!rows.length && !barren.length) return null;
            return (
                ...
                <table style={{ borderCollapse: "collapse", fontSize: 12.5 }}>
                  ...
                  <tbody>
                    {rows.map((c) => {
                      ...
                      // Direction ink from the shared palette rather than local hexes. The arrow glyph carries
                      // the same information, so colour here is redundant reinforcement and not the
                      // only channel; "flat" takes the neutral ink because no direction was established.
                      const dc = sb.sign === "positive" ? PAL.painHigh
                        : sb.sign === "negative" ? PAL.painLow : PAL.neutral;
                      ...
                      return (
                        <tr key={c.short} style={{ borderBottom: "1px solid #eef1f4" }}>
                          ...
                          {/* The FDR column has THREE states, not two. A q that was computed and
                              cleared 0.05 is a positive result; a q that was computed and did not
                              clear it is a negative result; and no q at all — which happens when
                              the clustered logistic could not be fitted at this band — is neither,
                              because nothing was tested. The first two used to share the tick
                              glyph's presence or absence and the third printed an em-dash in the
                              same grey as a genuine negative, so a reader could not tell an
                              untested band from a tested one that failed. Each state now carries
                              its own word, and the two non-positive states are separated by
                              wording rather than by shade alone. */}
                          <td style={{ padding: "2px 10px 2px 0",
                                       fontWeight: sb.fdr_significant ? 700 : 400,
                                       color: sb.q == null ? PAL.neutral : PAL.onWarn }}>
                            {sb.q == null
                              ? "not tested"
                              : `${sb.q.toFixed(3)}${sb.fdr_significant ? " \u2713" : " (n.s.)"}`}
                          </td>
                        </tr>
                      );
                    })}
                    {/* Channels the scan screened and for which no band produced a computable
                        correlation. Rendered as real rows in the neutral ink so the contact is
                        visibly present and visibly unresolved, rather than absent. */}
                    {barren.map((c) => (
                      <tr key={`barren-${c.short}`} style={{ borderBottom: "1px solid #eef1f4" }}>
                        <td style={{ padding: "2px 10px 2px 0", fontWeight: 600, color: PAL.neutral }}>
                          {c.short}
                        </td>
                        <td colSpan={5} style={{ padding: "2px 10px 2px 0", color: PAL.neutral }}>
                          {"screened, but no band in the sweep produced a computable correlation"}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                <MDTypography variant="caption" display="block" mt={0.3} sx={{ fontSize: 11, fontStyle: "italic", color: PAL.neutral }}>
                  {"\u2713 = survives band\u00d7channel BH-FDR (q<0.05). \u201Cn.s.\u201D = a q was computed "
                   + "and did not clear 0.05, so this is the contact's strongest exploratory band rather "
                   + "than a validated one. \u201Cnot tested\u201D = no q could be computed at that band, "
                   + "which is an absence of evidence and not a negative result. Signed AUC <0.5 = power "
                   + "falls with pain."}
                </MDTypography>
              </MDBox>
            );
...
export default function BiomarkerAnalytics({ analytics, summary, metricLabel, recordedPowers, programmedThresholds,
  ...
  const chSuffix = safeChSel === "pooled" ? ""
    : (isHemiSel ? ` · ${currentTarget(selHemi, `${selHemi} hemisphere`)}` : ` · ${currentTargetText(safeChSel)}`);
  ...
  const powerProvenance = (() => {
    if (isHemiSel) {
      ...
      return `${currentTarget(selHemi, `${selHemi} hemisphere`)} (${ks.length} contact${ks.length === 1 ? "" : "s"})${hzText ? ` @ ${hzText}` : ""}`;
    }
    if (safeChSel !== "pooled") {
      const hz = recHzFor(safeChSel);
      return `${currentTargetText(safeChSel)}${hz != null ? ` @ ${Number(hz).toFixed(1)} Hz` : ""}`;
    }
  ...
  if (scan && scan.channels && scan.channels.length) {
    tdPanels.push(
      <SpectralFeatureImportance key="sfi" scan={scan} pain={pain} HI={HI} LO={LO}
        ...
        onBandCommitted={onBandCommitted} />
    );
  }
    ...
    scs.channels.forEach((ch, i) => {
      ...
      tdPanels.push(
        <Panel key={"scs" + i} lg={12}
          title={`Sliding correlation with ${pain} (R: frequency × time) — ${currentTargetText(ch.channel)}`}>
      ...
      const drawSide = (h) => {
        ...
        const mk = (xc, field, keysAll, showLegend) => {
          ...
          barTraces.push({
            ...
            name: `${currentTarget(h)} contacts`, legendgroup: `swarm-${h}`, showlegend: showLegend,
    ...
    groupHemis.forEach((h) => {
      ...
      curves.forEach((c, i) => {
        rocTraces.push({ x: c.r.fpr, y: c.r.tpr, name: `${c.k} (AUC=${(c.r.auc ?? 0).toFixed(3)})`,
          ...
          legendgroup: h, legendgrouptitle: i === 0 ? { text: `${currentTarget(h, `${h} hemisphere`)}` } : undefined,
      ...
      if (curves.length >= 2) {
        ...
        if (mean) {
          rocTraces.push({ x: mean.fpr, y: mean.tpr,
            name: `${currentTarget(h)} mean of ${curves.length} contacts (AUC=${meanAuc.toFixed(3)})`,
            type: "scatter", mode: "lines", line: { width: 4, color: meanColorFor(h) },
            legendgroup: h,
            hovertemplate: `${currentTarget(h)} mean ROC<br>FPR=%{x:.2f} · TPR=%{y:.2f}<extra></extra>` });
          ...
          if (progThr && selHemiForProg === h) pushProgrammed(mean, `${currentTarget(h)} mean`);
        }
      }
    });
  ...
  if (rocTraces.length) {
    ...
    const rocProvenance = (() => {
      ...
      if (isContactView) { const hz = recHzFor(safeChSel); return `${currentTargetText(safeChSel)}${hz != null ? ` @ ${Number(hz).toFixed(1)} Hz` : ""}`; }
        ...
        <MDTypography variant="caption" color="dark" display="block" mt={1} sx={{ fontSize: 11 }}>
          {(powerProvenance ? `Power signal from ${powerProvenance}. ` : "") +
           ...
           (distOpInRange
             ...
             : `The ROC-optimal (Youden) threshold for closed-loop research review is shown on the ROC panel above.`)}
        </MDTypography>
      </Panel>
    );
  }
  ...
  if (selFreqDecode && selFreqDecode.binarization
    ...
    chPanels.push(
      <Panel key="freq-binarization"
        ...
        />
        <MDTypography variant="caption" color="dark" display="block" mt={1} sx={{ fontSize: 11 }}>
          {`Pain-score days available for ${currentTargetText(safeChSel)} @ ${Number(freqSel).toFixed(1)} Hz: ` +
