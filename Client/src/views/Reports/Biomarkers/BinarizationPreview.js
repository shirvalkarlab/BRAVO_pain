 ...
 * PRO<->PSD match window, with the strategy's high/low cuts and class counts shown above the chart.
 *
 ...
 *   * Cut lines + percentile labels above the plot area; class counts in responsive cards above the plot.
 */
...
function BinarizationPreview({ points, dailyAgg, strategy, percentileLow, percentileHigh,
  ...
  // Keep the existing class counts outside the chart so they can wrap without covering data.
  const classSummaries = useMemo(() => {
    ...
    if (cuts.kind === "none") return [];
    return [
      { label: "Low", color: LO, lines: lowTxt },
      ...(cuts.kind === "two-cut" ? [{ label: "Excluded", color: MID, lines: midTxt }] : []),
      { label: "High", color: HI, lines: highTxt },
    ];
  }, [matchedMode, counts, scanModel, dailyStats, cuts.kind, dir]);
  ...
  useEffect(() => {
    ...
    // Class counts render above the plot; reserve a little headroom above the tallest bar.
    ...
    const layout = {
      ...
      font: { family: "Roboto, Helvetica, Arial, sans-serif", size: 14, color: "#344767" },
      margin: { l: 48, r: 16, t: 54, b: 40 },
      bargap: 0.02,
      xaxis: { automargin: true, nticks: 5, title: { text: metricLabel || "Pain score", font: { size: 13 }, standoff: 8 },
               gridcolor: "#EEF1F4", linecolor: "#B0B7BF", ticks: "outside", ticklen: 4,
               tickfont: { size: 12 }, showline: true },
      yaxis: { automargin: true, nticks: 5, title: { text: yTitle, font: { size: 13 }, standoff: 8 },
               gridcolor: "#EEF1F4", linecolor: "#B0B7BF", ticks: "outside", ticklen: 4,
               tickfont: { size: 12 }, showline: true,
               ...(matchedMode && cuts.kind === "two-cut" ? { range: [0, yMax * 1.1] } : {}) },
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => {
      ...
      frame = requestAnimationFrame(() => { if (node?._fullLayout) Plotly.Plots.resize(node); });
    });
  ...
  const totalTxt = Number.isFinite(totalReports) && totalReports > 0
    ? ` · ${totalReports.toLocaleString()} eligible dated reports in the record` : "";
  ...
  return (
    <MDBox display="flex" flexDirection="column" sx={{ width: "100%", height: "100%", minHeight: 440 }}>
      <MDBox display="flex" flexDirection="row" justifyContent="space-between" alignItems="baseline" flexWrap="wrap" gap={1} mb={0.25}>
          ...
          {(loading || matchedLoading)
            ...
            : (scanModel && scanModel.matchable)
              ? `None of the ${(scanModel.counts && scanModel.counts.n_sessions) || 0} available `
                + `neural samples were eligible to pair with a pain rating at the \u00b1${matchTolerance} min window, so nothing `
                + "can be binarized under the current matching rules. Review the window and matching controls above. The histogram "
      ...
      <MDBox role="group" aria-label="Binarization class counts" sx={{display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 1, mb: 1}}>
        {classSummaries.map(({label, color, lines}) => (
          <MDBox key={label} sx={{p: 1, borderRadius: 1, backgroundColor: color, color: "white", minWidth: 0, overflowWrap: "anywhere", textAlign: "center"}}>
            <MDTypography variant="button" color="inherit" fontWeight="bold" display="block">{label}</MDTypography>
            {lines.filter(Boolean).map((line, index) => <MDTypography key={index} variant="caption" color="inherit" display="block">{line}</MDTypography>)}
          </MDBox>
        ))}
      </MDBox>
      <MDBox role="region" aria-label="Binarization histogram" tabIndex={0} sx={{flex: 1, width: "100%", minWidth: 0}}>
        <div ref={ref} style={{ width: "100%", minWidth: 0, minHeight: 340 }} />
      </MDBox>
