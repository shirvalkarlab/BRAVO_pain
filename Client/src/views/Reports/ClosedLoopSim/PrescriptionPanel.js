...
export default function PrescriptionPanel({ report, mode, onMode }) {
  ...
  if (!data || (data.planning && data.planning.available === false) || !pres || !pres.modes) {
    return (
      <Card><MDBox p={2}>
        <MDTypography variant="h6" sx={{ fontSize: 15 }}>Research parameter plan</MDTypography>
        <MDTypography variant="caption" sx={{ display: "block", fontSize: 11.5,
          color: PAL.neutral }}>
          {(data && data.planning && data.planning.reason) || `No participant-specific parameter plan is available${err ? ` (${err})` : ""}.`}
          {" This research page does not authorize programming or infer missing device settings."}
        </MDTypography>
        <MDBox mt={1} display="flex" gap={1} flexWrap="wrap">
          {MODE_ORDER.map((choice) => <MDButton key={choice} size="small" variant={mode === choice ? "contained" : "outlined"}
            color="info" onClick={() => onMode && onMode(choice)}>{MODE_LABEL[choice]}</MDButton>)}
        </MDBox>
        <MDTypography variant="caption" display="block" mt={1}>
          Selected mode: {MODE_LABEL[mode] || MODE_LABEL.dual}. Settings, thresholds, capture limits and timing remain unavailable.
        </MDTypography>
      </MDBox></Card>
    );
  }
  ...
  const deviceOk = !!(data.available && data.licensed === true && data.readiness && data.readiness.ready === true && vd.device_eligible === true);
