...
export default function DutyCyclePanel({ report, mode }) {
  ...
  const modeCanDrive = !(data && data.planning && data.planning.available === false)
    && (!modeBlock || (modeBlock.fields || []).length > 0);
    ...
    return (
      <Card sx={{ border: "2px dashed rgba(0,0,0,0.25)" }}><MDBox p={2}>
        ...
        <MDTypography variant="caption" sx={{ display: "block", fontSize: 11.5,
          color: PAL.neutral }}>
          {data && data.planning && data.planning.available === false ? data.planning.reason : duty === null && data
            ? `No duty cycle is computed for ${MODE_LABEL[activeMode] || activeMode}`
              + `${activeMode === "single_inverse" && modeBlock && !(modeBlock.fields || []).length
