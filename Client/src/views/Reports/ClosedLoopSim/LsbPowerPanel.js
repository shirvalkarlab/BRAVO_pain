 ...
 *   1) THRESHOLD FOR RESEARCH REVIEW — the percentile-anchored device-LSB threshold (the deployable number),
...
function LsbPowerPanel({ autoFetch = false, inputIdentity, participantUid, bandCandidate, requestParams, cutpoint, onLsbThreshold,
                         deploymentReport }) {
  ...
  // the device's own Timeline band power falls — but it labels that number "THRESHOLD FOR RESEARCH REVIEW"
  ...
  const matchDir = cutpoint ? cutpoint.matchDir : ((requestParams && requestParams.MatchDirection) || "prior");
  ...
  const canRun = !!participantUid && channelRaw != null && centerHz != null && cutThr != null;
  const cached = useCachedResult({
    ...
    enabled: canRun,
    autoFetch,
    fetcher: () => queryAnalysis("/api/queryLsbPower",
  ...
  useEffect(() => {
    ...
    onLsbThreshold(!cached.stale && canRun && tlUpperLsb != null && Number.isFinite(Number(tlUpperLsb))
      ? { upperLsb: Number(tlUpperLsb), estimated: tlEstimated } : null);
  }, [tlUpperLsb, tlEstimated, cached.stale, canRun]);  // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <Card sx={{ width: "100%" }}>
      <MDBox p={2}>
        ...
        <PanelStaleNote stale={cached.stale} staleReasons={cached.staleReasons}
          ...
          onRecompute={cached.recompute} hasResult={cached.hasCached} disabled={!canRun} />
        ...
        {cutThr == null ? (
          ...
          <>
            ...
            {cutDegenerate ? (
              <MDBox p={1.2} mb={1.2} sx={{ backgroundColor: PAL.warnFill, borderRadius: "6px",
                ...
                <MDTypography variant="caption" sx={{ fontSize: 11, fontWeight: "bold", color: PAL.warnText }}>
                  ...
                  panel and choose a balanced cut-point before clinical review.
                </MDTypography>
              </MDBox>
            ) : null}

            {/* 1) THRESHOLD FOR RESEARCH REVIEW — MEASURED (native device Timeline) */}
            {tl && tl.available && !tl.estimated ? (
              <MDBox p={1.2} mb={1.2} sx={{ backgroundColor: PAL.accentFill, borderRadius: "6px",
                ...
                <MDTypography variant="caption" sx={{ fontSize: 10, fontWeight: "bold",
                  ...
                  {deviceBlocks
                    ...
                    : "THRESHOLD FOR RESEARCH REVIEW (device LSB)"}
                </MDTypography>
              ...
              /* THRESHOLD FOR RESEARCH REVIEW — ESTIMATED (modeled fallback: device never sensed this band).
