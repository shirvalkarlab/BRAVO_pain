...
const shortChannel = (ch) => (ch || "")
  ...
  .replace("_RIGHT", ` ${currentTarget("Right", "R")}`).replace("_LEFT", ` ${currentTarget("Left", "L")}`);

function ConversionModelPanel({ autoFetch = false, inputIdentity, participantUid }) {
  ...
  const cached = useCachedResult({
    ...
    enabled: canRun,
    autoFetch,
    fetcher: () => queryAnalysis("/api/queryPsdLsbConversionModel",
