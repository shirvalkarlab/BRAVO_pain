 ...
 */
function TriangleGraph({ edges, outcome }) {
  const W = 330;
  const H = 210;
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} role="img"
      ...
      {[["amp", "Amplitude", "mA", "middle", 0, 22],
        ...
        ["pain", "Pain", outcome || "recorded outcome", "middle", 0, 22]].map(([key, label, unit, anchor,
        ox, oy]) => (
          <g key={key}>
...
function EdgeAxis({ k, e }) {
  ...
  const est = e && e.estimate != null && Number.isFinite(Number(e.estimate)) ? Number(e.estimate) : null;
  const resolved = !!(e && e.resolved);
  const ink = edgeInk(e);
  ...
  const yAxis = 34;
  const loX = lo.unbounded ? x0 : lo.unavailable ? zero : px(lo.value);
  const hiX = hi.unbounded ? x1 : hi.unavailable ? zero : px(hi.value);

  const readout = e == null ? "not estimated"
    : `${fmtNum(est, 4)}  [${lo.unbounded ? "unbounded" : lo.unavailable ? "unavailable" : fmtNum(lo.value, 4)}, `
      + `${hi.unbounded ? "unbounded" : hi.unavailable ? "unavailable" : fmtNum(hi.value, 4)}]  p = ${fmtP(e.p)}`;
  ...
  return (
    <MDBox mb={0.6}>
      ...
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} role="img"
        ...
        {e && !lo.unavailable && !hi.unavailable ? (
    ...
    <MDBox mt={1}>
      ...
      <MDBox mt={1} p={1} sx={{ borderRadius: "4px", backgroundColor: PAL.neutralFill,
        ...
        <MDTypography variant="caption" sx={{ display: "block", fontSize: 11.5, mt: 0.6,
          ...
          {!r.haveAllSigns || !r.haveExpectedSigns
            ? "Cannot be answered while an observed or expected sign is missing."
            : r.matchesControlLaw
        ...
        {r.haveAllSigns && r.haveExpectedSigns && r.edgesAgreeInternally && !r.matchesControlLaw ? (
          <MDTypography variant="caption" sx={{ display: "block", fontSize: 11.5, mt: 0.6,
            color: PAL.warnText }}>
      ...
      <MDBox p={2}>
        ...
        <Grid container spacing={2} mt={0.5}>
          <Grid item xs={12} md={5}>
            <TriangleGraph edges={edges} outcome={data.manifest && data.manifest.outcome} />
            <MDTypography variant="caption" sx={{ display: "block", fontSize: 10.5,
              color: "#8A8A8A", mt: 0.5 }}>
