...
function PsdLsbPanel({ autoFetch = false, inputIdentity, participantUid, bandCandidate, requestParams }) {
  ...
  const canRun = !!participantUid && channelRaw != null;
  const cached = useCachedResult({
    ...
    enabled: canRun,
    autoFetch,
    fetcher: () => queryAnalysis("/api/queryPsdLsbConversion",
