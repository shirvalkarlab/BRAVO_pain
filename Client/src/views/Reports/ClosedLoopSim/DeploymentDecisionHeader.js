...
export default function DeploymentDecisionHeader({ bandCandidate, summary, deploymentReport }) {
  ...
  const v = !rep || !rep.licensed || !rep.readiness || rep.readiness.ready !== true
    ? { ink: PAL.neutral, icon: "science", headline: "Closed-loop research review",
        body: "Retrospective evidence and encoded device checks are separate findings. This page does not establish clinical readiness or authorize device programming." }
    : reconcile(device, evidence, transcription);
        ...
        <MDBox display="flex" flexDirection="row" alignItems="flex-start" gap={1.2}>
          ...
          <MDBox flex="1 1 auto">
            ...
            <MDTypography variant="caption" sx={{ display: "block", fontSize: 10.5, mt: 0.4,
              color: "#7A7A7A" }}>
              {`${currentTargetText(bc.contact_label || bc.contact || "band")} at `
                + `${fmtHz(bc.center_freq_hz) || "an unspecified"} Hz`
                + `${bc.hemisphere ? `, ${currentTarget(bc.hemisphere)}` : ""}`
