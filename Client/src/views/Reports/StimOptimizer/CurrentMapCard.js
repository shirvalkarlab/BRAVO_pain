...
export function CurrentSurfaceHeatmap({ divId, surface, inForceLeft, inForceRight, starLeft, starRight,
  showStar, size = 400 }) {
  const figRef = useRef(null);
  const { ref: containerRef, width } = useResizeDetector();
  const [renderError, setRenderError] = useState(false);
  const chartSize = Math.min(size, Math.max(1, width || size));
  ...
  useEffect(() => {
    ...
    fig.setLayoutProps({
      height: chartSize, width: chartSize, margin: { l: 46, r: 20, t: 8, b: 40 },
    ...
    try {
      Promise.resolve(Plotly.react(divId, fig.traces, fig.layout,
  ...
  return <div ref={containerRef} style={{ width: "100%", maxWidth: size, minWidth: 0 }}>
    {renderError && <MDTypography role="alert" variant="body2">The current map could not be displayed. Retry the analysis.</MDTypography>}
    <div id={divId} role="img" aria-label="Current map" style={{ width: chartSize, height: chartSize, minWidth: 0 }} />
  </div>;
}
