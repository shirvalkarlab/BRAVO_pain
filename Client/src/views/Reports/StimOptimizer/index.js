 ...
 */
export function FigurePanel({ title, blurb, figure, prominence, error }) {
  const {ref, width = 1024} = useResizeDetector({refreshMode: "debounce", refreshRate: 80});
  const rendered = figure ? responsiveFigure(figure, width) : null;
  const primary = prominence === "primary";
  const hasFigure = !!figure;
  useEffect(() => {
    const gd = ref.current;
    if (!gd || !figure) return;
    Plotly.react(gd, rendered.data, {...rendered.layout, height: Math.max(primary ? 620 : 380, rendered.layout.height || 0)}, MODEBAR);
  }, [figure, width, primary]);
  // Capture the mounted graph node: React clears ref.current before passive cleanup.
  useEffect(() => {
  ...
  return (
    <MDBox mb={primary ? 4 : 3}>
      ...
      {rendered.notes.map((note, index) => <MDTypography key={index} variant="caption" color="text" component="div" sx={{mb: 1, overflowWrap: "anywhere"}}>{note}</MDTypography>)}
      <MDBox role="region" aria-label={title} tabIndex={0} sx={{width: "100%", minWidth: 0}}>
        <MDBox ref={ref} sx={{ width: "100%", minWidth: 0, minHeight: primary ? 620 : 380 }} />
      </MDBox>
    </MDBox>
  );
}
...
export default function StimOptimizer() {
  ...
  useEffect(() => { setContextState(dispatch, "report", "CustomizedAnalysis"); }, [participant_uid]);
  ...
  // Access and data identity are checked without starting a scientific analysis.
  if (loading) {
    return (
      <DatabaseLayout>
        <MDBox pt={3} display="flex" alignItems="center" justifyContent="center" gap={2}>
          ...
          <MDTypography variant="body2">
            Loading the requested view and checking its data identity…
          </MDTypography>
        </MDBox>
      </DatabaseLayout>
    );
  ...
  if (!data && !errorText) {
    return <DatabaseLayout><Card sx={{p:{xs:2,md:3},mt:3}}>
      <MDTypography variant="h4">Open-Loop Stim Optimizer</MDTypography>
      <MDTypography variant="body2" sx={{my:2}}>No validated result is currently retained for this view. Start analysis to fit the parameter surfaces; opening this page does not launch a scientific job.</MDTypography>
      <MDButton variant="contained" color="info" onClick={cached.recompute}>Start analysis</MDButton>
    </Card></DatabaseLayout>;
  }
              ...
              <Card>
                <MDBox p={2}>
                  <MDBox display="flex" alignItems="center" justifyContent="space-between">
                    <MDTypography variant="h6">Closed-loop research preparation</MDTypography>
          ...
          <Grid item xs={12}>
            <Card>
              <MDBox p={2}>
                ...
                <MDBox sx={{overflowX: "auto", width: "100%"}} tabIndex={0} role="region" aria-label="Scrollable analysis table">
<Table size="small" sx={{ mt: 1 }}>
                ...
                <MDTypography variant="caption" color="text" component="div" sx={{ mt: 1 }}>
                  ...
                  assigned its resolution verdict by the backend using the declared uncertainty
                  margin. The displayed interval is one standard deviation of the difference,
                  propagated from both cells&apos; posteriors as{" "}
                  ...
                  deviation omits the <em>&minus;2&thinsp;cov</em> term. This overstates uncertainty
                  when that covariance is positive; without the joint covariance, conservatism
                  cannot be guaranteed for every comparison.
                </MDTypography>
