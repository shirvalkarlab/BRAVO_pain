...
def td_chan_to_contact(ch):
    ...
    for pair in ("ZERO_AND_THREE","ONE_AND_THREE","ZERO_AND_TWO","ONE_AND_TWO","ZERO_AND_ONE","TWO_AND_THREE"):
        if pair in s: return pair,("RIGHT" if "RIGHT" in s else "LEFT")
    return None,None
def psd_contact(e):
...
for s in surveys:
    ...
    for ci,ch in enumerate(chans):
        ...
        lf,lm=psdmap[key]
        for c in CENTERS:
            tdbp=analytics.td_transform_band_power(col,fs,float(c),half_hz=HALF)
            # NOW uses the CORRECTED (closed-edge, vectorized) device_psd_band_power
            pbp=analytics.device_psd_band_power(lf,lm,float(c),half_hz=HALF)
            if np.isfinite(tdbp) and np.isfinite(pbp) and tdbp>0 and pbp>0:
                logratios.append(np.log(pbp)-np.log(tdbp))
lr=np.array(logratios)
print(f"n={lr.size}")
print(f"K_TD_PSD with CLOSED-edge device helper: geomean={np.exp(np.mean(lr)):.4f}  median={np.exp(np.median(lr)):.4f}")
print(f"  (constant in code = {analytics.LSB_PER_UV2_DEVICE_PSD_TD_RATIO})")
print(f"  diff from 4.789: {abs(np.exp(np.mean(lr))-4.789):.4f}  ({100*abs(np.exp(np.mean(lr))-4.789)/4.789:.2f}%)")
