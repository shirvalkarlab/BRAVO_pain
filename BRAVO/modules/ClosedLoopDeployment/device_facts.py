...
def impedance_facts(recordings):
    ...
    Returns both the minimum and maximum bipolar readings per hemisphere, because D16 is a
    fault check: one short or open matters even when the other pairs are healthy. Triangular
    matrix zero placeholders are not measurements and are excluded. The device's own Status field is carried through
    ...
    for hemi in ("Left", "Right"):
        ...
        out[hemi] = {
            "lead_model": h.get("LeadModel"),
            "lead_type": _lead_type(h.get("LeadModel")),
...
def candidate_impedance_ohm(facts, hemisphere):
    """The maximum bipolar reading D16 compares against its open-circuit limit.
...
def facts_for_participant(participant_uid, impedance_recordings=None, *,
                          hemisphere=None, channel=None, stated_facts=None, session_summary=None):
    ...
    if paused and hemisphere in (paused or {}):
        ...
        prov["paused_amplitude_mA"] = f"caller-supplied reviewed declaration for the {hemisphere} hemisphere"
    ...
    if imp.get("available"):
        ...
        if ohm is not None:
            ...
            prov["impedance_ohms"] = (
                f"measured: maximum bipolar pair in the NEWEST of {imp['n_records']} impedance "
            ...
            prov["impedance_min_ohms"] = (
                f"measured: minimum non-placeholder bipolar pair in the NEWEST of "
                f"{imp['n_records']} impedance recordings on the {hemisphere} lead; "
                "zero triangle placeholders excluded")
        out["impedance_tested"] = True
        prov["impedance_tested"] = f"measured: {imp['n_records']} impedance recordings on record"
        lead = imp.get(hemisphere) or {}
        if lead.get("lead_type"):
            out["lead_type"] = lead["lead_type"]
            prov["lead_type"] = f"measured: {hemisphere} LeadModel {lead.get('lead_model')}"
    ...
    srf, srf_prov = session_report_facts_for(participant_uid, channel=channel,
                                             hemisphere=hemisphere, summary=session_summary)
...
def session_report_facts_for(participant_uid, *, channel=None, hemisphere=None, summary=None):
