...
from modules.ClosedLoopDeployment import edges as _edges
...
def _frame_fingerprint(df, columns=(), *, either=(), at_least_one=(), also=()):
    ...
    if df is None:
        return ("none",)
    columns = list(columns) + list(also) + [c for group in (*either, *at_least_one) for c in group]
        ...
        if isinstance(first, (np.ndarray, list, tuple)):
            try:
                ...
                for v in col.to_numpy():
                    array = np.ascontiguousarray(np.asarray(v, dtype=float))
                    buf += repr(array.shape).encode("ascii") + b":" + array.tobytes()
                parts.append((c, _hashlib.blake2b(bytes(buf), digest_size=16).hexdigest()))
            except Exception:
                degraded = True
        else:
            try:
                hashed = pd.util.hash_pandas_object(col, index=True).to_numpy().tobytes()
                parts.append((c, _hashlib.blake2b(hashed, digest_size=16).hexdigest()))
            except Exception:
                degraded = True
...
def _joined_signature(psd_frame, epochs, centers, width, pro_frame=None):
    ...
    return (_frame_fingerprint(psd_frame, tuple(["t", "channel", "source", "log_psd", "freqs", "band_half_hz", "tile_ok", "tile_saturated", "tile_window_s"] + [str(c) for c in getattr(psd_frame, "columns", ()) if str(c).startswith(_CAL_LSB_PREFIX)])),
            _frame_fingerprint(epochs, ("t_start", "t_end", "amp_mA_Left", "amp_mA_Right",
                                        "amp_Left", "amp_Right", "freq_hz", "pw_us_Left", "pw_us_Right",
                                        "pw_Left", "pw_Right", "cathode_Left", "cathode_Right",
                                        "epoch", "dur_h", "open_ended")),
            _frame_fingerprint(pro_frame, list(pro_frame.columns) if pro_frame is not None else ()) ,
...
def recording_set_signature(participant):
    """Include approved source content, clocks, processing policy and corrected PROs."""
    from modules.AnalysisData import input_manifest
    manifest = input_manifest(participant)
    return (str(getattr(participant, "uid", participant)), manifest.get("source_count", 0),
            manifest.get("recordings", 0), manifest["fingerprint"])
...
def amplitude_response_cached(steps, tiles_by_channel, *, centers_hz,
    ...
    """
    from modules.StimOptimizer.routines import lfp_evidence as _ev
    from modules.StimOptimizer.routines import within_visit as _wv
    if response_fn is None:
        from modules.StimOptimizer.routines import lfp_response as _lr
...
def joined_table_cached(psd_frame, epochs, *, centers=None, width=DEFAULT_BAND_WIDTH_HZ,
    ...
    """
    cen = tuple((calibrated_centres(psd_frame) or DEFAULT_BAND_CENTERS_HZ) if centers is None else centers)
    sig = _joined_signature(psd_frame, epochs, cen, width, kwargs.get("pro_frame"))
    # An unhashable input must never reuse a shape-only result.
    if any(part[0] == "shape_only" for part in sig[:3]):
        return joined_table(psd_frame, epochs, centers=cen, width=width, **kwargs)
    if not force_refresh:
    ...
    for i, (_, r) in enumerate(psd_frame.iterrows()):
        ...
        for c in centers:
            rows.append({
                ...
                "setting_epoch": ctx.get("epoch", e), "center_hz": float(c), "band_width_hz": float(width),
...
def _joined_table_calibrated(psd_frame, epochs, *, centers=DEFAULT_BAND_CENTERS_HZ, pro_frame=None):
    ...
    for c in centers:
        ...
        block = {"t": t, "channel": chan, "source": src, "setting_epoch": ctx_cols.get("epoch", ep_idx),
...
def scale_disagreement(T):
    ...
    """
    if T is None or T.empty or T[["channel", "center_hz"]].drop_duplicates().shape[0] < 2:
        return {"available": False, "reason": "At least two band cells are required to compare rankings across power scales"}
...
def report_to_dict(rep):
    ...
    """
    el = rep.eligibility
    device_ok = (False if el is not None and el.failures else
                 None if el is None or el.unknowns else bool(el.eligible))
...
def simulation_inputs_for_participant(uid, *, contact, centre_hz, hemisphere, epochs=None,
    ...
    from modules.Biomarkers.routines import availability as _avail
    from . import three_source_response as _3src
...
def report_for_participant(participant, request_data=None, *, candidates=None, hemisphere="Left",
    ...
    """
    if not hasattr(participant, "uid"):
        from .bravo_service import run_for_participant
        first = (candidates or [{}])[0]
        controls = dict(request_data or {})
        controls.update(ParticipantId=str(participant), Channel=first.get("channel"),
                        CenterHz=first.get("center_hz"), BandWidthHz=first.get("band_width_hz", 5))
        return run_for_participant(controls)
    from modules.StimOptimizer import adapter as _sa
    ...
    try:
        from modules.ClosedLoopDeployment import device_facts as _df_ag
    ...
    rep = _pl.run(getattr(participant, "uid", participant), psd_frame=psd, epochs=eps,
                  ...
                  place_thresholds=_place, outcome=outcome, outcome_cluster="setting_epoch",
                  participant_context=rd.get("_participant_context"),
                  band_width_hz=float((cands[0] or {}).get("band_width_hz", 5)),
                  washin_s=float(rd.get("WashinMin", 1)) * 60)
    ...
    try:
        ...
        if _ch is not None and _fc is not None:
            ...
            _core = _bsvc._validate_band_core({
                ...
                **rd, "Channel": _ch, "CenterHz": float(_fc), "BandWidthHz": _bw,
            })
    ...
    try:
        ...
        from modules.StimOptimizer.adapter import PRO_ITEMS as _PRO_ITEMS
