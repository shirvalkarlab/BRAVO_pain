...
def _assign_epoch(t_epoch_s, epochs):
    ...
    for i, (a, b) in enumerate(zip(starts, ends)):
        open_ended = bool(epochs.iloc[i].get("open_ended", False))
        m = (t >= a) if open_ended or not np.isfinite(b) else (t >= a) & (t < b)
        out[m] = i
    return out
...
def scale_disagreement(T):
    ...
    if T.center_hz.nunique() < 2:
        return {"available": False, "reason": "A single selected band cannot establish peak-band disagreement"}
...
def report_to_dict(rep):
    ...
    """
    el = rep.eligibility
    device_ok = (bool(el.eligible) if el is not None and (el.failures or not el.unknowns) else None)
    ...
    if el is not None and el.failures:
        verdict = "blocked"
...
def report_for_participant(participant, request_data=None, *, candidates=None, hemisphere="Left",
    ...
    """Compatibility entrypoint routed through Aditya's canonical research-only service.

    Candidate observations and programmer facts are never accepted through this seam.
    """
    from . import bravo_service
    request = dict(request_data or {})
    request["ParticipantId"] = str(getattr(participant, "uid", participant))
    candidate = next(iter(candidates or request.pop("Candidates", []) or []), {})
    for source, target in (("channel", "Channel"), ("center_hz", "CenterHz"),
                           ("band_width_hz", "BandWidthHz")):
        if source in candidate:
            request.setdefault(target, candidate[source])
    return bravo_service.run_for_participant(request)
