...
def _participant_facts(participant_uid, device_facts=None, constraints_module=None,
    ...
    """
    # Only the canonical caller can supply participant-specific declarations.
...
def run(participant_uid, *, psd_frame=None, epochs=None, design_matrix=None, pro_frame=None,
        ...
        hemisphere="Left", strict=True, n_boot=500, seed=0, participant_context=None,
    ...
    centers = tuple(float(c["center_hz"]) for c in cand) or adapter.DEFAULT_BAND_CENTERS_HZ
    T = adapter.joined_table_cached(psd_frame, epochs, pro_frame=pro_frame,
                             centers=centers, width=band_width_hz)
    if not T.empty:
    ...
    e3 = E.therapy_edge(design_matrix, outcome=outcome, amp_col=adapter.canonical_amp_col(hemisphere),
                        n_boot=n_boot, seed=seed)
    ...
    # Participant-specific plans are optional; canonical Aditya research requests leave them disabled.
    if include_planning:
