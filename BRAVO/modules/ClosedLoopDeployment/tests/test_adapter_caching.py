...
def _clinic_steps(n_visits=4, n_amps=3):
    """A parsed clinic testing sheet in the columns the estimator reads.

    Amplitude varies WITHIN each visit and the visits sit on different days, which is the design
    that makes blocking on visit informative: it removes calendar time without also removing the
    contrast in delivered current.
    """
    rows = []
    t = 1_750_000_000.0
    for v in range(n_visits):
        for a_i, amp in enumerate(np.linspace(1.0, 3.0, n_amps)):
            rows.append({"t0": t + v * 86400.0 + a_i * 120.0, "window_s": 90.0,
                         "visit": f"2025-08-{v + 1:02d}", "rate_hz": 110.0,
                         "amp_mA_Left": float(amp), "amp_mA_Right": np.nan,
                         "pw_us_Left": 60.0})
    return pd.DataFrame(rows)


def _tiles(steps, n_centers=6, seed=0):
    """Three-second tile power that falls as delivered current rises, so a fit has something to find."""
    rng = np.random.default_rng(seed)
    ts, ps = [], []
    for _, r in steps.iterrows():
        n = 20
        tt = r["t0"] + 46.0 + np.arange(n) * 3.0
        base = 10.0 - 1.5 * float(r["amp_mA_Left"])
        ps.append(base + rng.normal(0.0, 0.05, size=(n, n_centers)))
        ts.append(tt)
    return {"ZERO_THREE_LEFT": (np.concatenate(ts), np.vstack(ps))}
...
# ---------------------------------------------------------------------------------------------
# current against band power, for every band, electrode and visit
# ---------------------------------------------------------------------------------------------
def test_the_same_clinic_sheet_twice_fits_nothing_the_second_time():
    """The property the PI asked for: worked out once, then reused.

    Counted rather than timed. A timing comparison on a small sheet is noisy enough to pass by
    luck, whereas counting how many times the per-band fit is called cannot.
    """
    steps = _clinic_steps()
    tiles = _tiles(steps)
    calls = {"n": 0}
    ...
    def counted(power, amplitude, **kw):
        calls["n"] += 1
        return LR.assess_response(power, amplitude, **kw)

    first = AD.amplitude_response_cached(steps, tiles, centers_hz=np.arange(10.5, 16.5, 1.0),
                                         response_fn=counted, hemispheres=("Left",))
    after_first = calls["n"]
    assert after_first > 0, "the first request must actually fit something, or this proves nothing"
    second = AD.amplitude_response_cached(steps, tiles, centers_hz=np.arange(10.5, 16.5, 1.0),
                                          response_fn=counted, hemispheres=("Left",))
    assert calls["n"] == after_first, "the second request must not fit a single band again"
    assert second is first, "and it must hand back the very same result"
    assert AD.response_cache_stats()["entries"] == 1


def test_an_edited_clinic_sheet_makes_the_figures_be_worked_out_again():
    """The other half, and the reason the key hashes contents rather than counting rows.

    One delivered current corrected from 3.0 to 2.75 mA changes nothing about the sheet's size or
    the dates on any file. This data has already been parsed several times with corrections of
    exactly this kind, including to which side of a bilateral entry is the left one, so serving the
    earlier figures here would be serving figures for a convention that has since been overturned.
    """
    steps = _clinic_steps()
    tiles = _tiles(steps)
    first = AD.amplitude_response_cached(steps, tiles, centers_hz=np.arange(10.5, 16.5, 1.0),
                                         hemispheres=("Left",))
    edited = steps.copy()
    edited.loc[edited.index[-1], "amp_mA_Left"] = 2.75
    assert len(edited) == len(steps), "the edit must not change the row count, or this is no test"
    second = AD.amplitude_response_cached(edited, tiles, centers_hz=np.arange(10.5, 16.5, 1.0),
                                          hemispheres=("Left",))
    assert second is not first, "an edited sheet must be recomputed"
    assert AD.clinic_steps_signature(edited) != AD.clinic_steps_signature(steps)


def test_a_new_visit_appended_to_the_sheet_also_invalidates():
    """The case the PI described in as many words: a new in-clinic visit arrives."""
    steps = _clinic_steps(n_visits=3)
    tiles = _tiles(steps)
    first = AD.amplitude_response_cached(steps, tiles, centers_hz=np.arange(10.5, 16.5, 1.0),
                                         hemispheres=("Left",))
    grown = _clinic_steps(n_visits=4)
    second = AD.amplitude_response_cached(grown, _tiles(grown),
                                          centers_hz=np.arange(10.5, 16.5, 1.0),
                                          hemispheres=("Left",))
    assert second is not first


def test_re_decoded_recordings_invalidate_even_though_the_sheet_is_untouched():
    """The same sheet read against different tile power is a different calculation.

    Keying on the sheet alone would serve figures computed from the previous decoding of the
    recordings, which is the same class of error as ignoring an edited sheet.
    """
    steps = _clinic_steps()
    first = AD.amplitude_response_cached(steps, _tiles(steps, seed=0),
                                         centers_hz=np.arange(10.5, 16.5, 1.0),
                                         hemispheres=("Left",))
    second = AD.amplitude_response_cached(steps, _tiles(steps, seed=1),
                                          centers_hz=np.arange(10.5, 16.5, 1.0),
                                          hemispheres=("Left",))
    assert second is not first


def test_nothing_else_invalidates_the_figures():
    """The other half of the PI's request: ONLY new data should prompt recalculation.

    A column added to the sheet downstream of the parse, and a row reordering, must both leave the
    cached figures in place, or the cache would rebuild constantly and be worth nothing. Note that
    the delivered values are unchanged in both cases.
    """
    steps = _clinic_steps()
    tiles = _tiles(steps)
    first = AD.amplitude_response_cached(steps, tiles, centers_hz=np.arange(10.5, 16.5, 1.0),
                                         hemispheres=("Left",))
    annotated = steps.copy()
    annotated["a_note_added_later"] = "not an input to any fit"
    second = AD.amplitude_response_cached(annotated, tiles, centers_hz=np.arange(10.5, 16.5, 1.0),
                                          hemispheres=("Left",))
    assert second is first, "an added annotation must not throw the figures away"


def test_the_figures_memo_stays_bounded():
    steps = _clinic_steps()
    tiles = _tiles(steps)
    for k in range(AD._RESPONSE_MEMO_MAX + 2):
        AD.amplitude_response_cached(steps, tiles,
                                     centers_hz=np.arange(10.5 + k, 16.5 + k, 1.0),
                                     hemispheres=("Left",))
    assert AD.response_cache_stats()["entries"] == AD._RESPONSE_MEMO_MAX


def test_force_refresh_recomputes_even_though_nothing_changed():
    """Needed while working on a response function, whose body can change without its name doing so."""
    steps = _clinic_steps()
    tiles = _tiles(steps)
    first = AD.amplitude_response_cached(steps, tiles, centers_hz=np.arange(10.5, 16.5, 1.0),
                                         hemispheres=("Left",))
    again = AD.amplitude_response_cached(steps, tiles, centers_hz=np.arange(10.5, 16.5, 1.0),
                                         hemispheres=("Left",), force_refresh=True)
...
# ---------------------------------------------------------------------------------------------
def test_a_second_process_finds_what_the_first_one_built(monkeypatch):
    """Emptying only the memo stands in for a request arriving at a different worker process.

    This is the condition that made the endpoint feel slow after the memo went in: four worker
    processes, each with its own memory, and every code edit replacing all four. A cache that is
    per-process makes "worked out once" true per process, which on this record meant four full
    ...
    first = AD.amplitude_response_cached(steps, tiles, centers_hz=np.arange(10.5, 16.5, 1.0),
                                         hemispheres=("Left",))
    assert AD.shared_cache_stats()["entries"] == 1, "the first request must have left a file"

    AD.clear_response_cache()                      # a different worker process: nothing in memory

    # Make the calculation itself impossible, so the second request can only succeed by reading the
    # file. Asserting on a count of fits would be weaker: a count of zero is also what a request
    # that quietly did nothing would report.
    from StimOptimizer.routines import within_visit as WV

    def refuse(*a, **k):
        raise AssertionError("this request rebuilt instead of reading the shared file")

    monkeypatch.setattr(WV, "build_all_within_visit", refuse)
    second = AD.amplitude_response_cached(steps, tiles, centers_hz=np.arange(10.5, 16.5, 1.0),
                                          hemispheres=("Left",))
    assert sorted(second["cells"]) == sorted(first["cells"]), \
        "and the shared result must be the same result"
    assert AD.shared_cache_stats()["events"]["hits"] >= 1


def test_a_corrupt_shared_file_is_a_miss_and_not_an_error():
    """A half-written or unreadable file must never take down a clinician's page.

    The correct answer is always still obtainable by rebuilding, so every failure reading a file is
    treated as though the file were not there, and the bad file is removed so it cannot be found
    again.
    """
    steps = _clinic_steps()
    tiles = _tiles(steps)
    AD.amplitude_response_cached(steps, tiles, centers_hz=np.arange(10.5, 16.5, 1.0),
                                 hemispheres=("Left",))
    d = AD.shared_cache_dir()
    import os
    files = [f for f in os.listdir(d) if f.endswith(".pkl")]
    assert files
    with open(os.path.join(d, files[0]), "wb") as fh:
        fh.write(b"this is not a pickle")

    AD.clear_response_cache()
    out = AD.amplitude_response_cached(steps, tiles, centers_hz=np.arange(10.5, 16.5, 1.0),
                                       hemispheres=("Left",))
    assert out["cells"] is not None, "it rebuilt instead of raising"
    assert AD.shared_cache_stats()["events"]["unreadable"] == 1


def test_a_file_whose_stored_key_does_not_match_is_ignored():
    """The file name holds a hash of the key, and a hash can collide.

    The key itself is stored inside the file and compared, so a mismatch causes a rebuild rather
    than a wrong answer.
    """
    AD._shared_store("inputs", ("a key",), "payload for a key")
    assert AD._shared_load("inputs", ("a key",)) == "payload for a key"
    import os
    d = AD.shared_cache_dir()
    f = [x for x in os.listdir(d) if x.endswith(".pkl")][0]
    os.rename(os.path.join(d, f), os.path.join(d, AD._shared_path("inputs", ("another key",))))
    assert AD._shared_load("inputs", ("another key",)) is None


def test_an_oversized_entry_is_refused_rather_than_filling_the_disk(monkeypatch):
    """A cache must never be the reason a machine holding a participant's recordings runs out of
    space. Refusing to write leaves the result in this process's memory, which is a slower cache
    and not a broken one."""
    monkeypatch.setattr(AD, "_SHARED_CACHE_MAX_BYTES", 64)
    assert AD._shared_store("inputs", ("big",), np.zeros(10_000)) is False
    assert AD.shared_cache_stats()["events"]["refused_too_big"] == 1
    assert AD._shared_load("inputs", ("big",)) is None
