...
contestant D's and contestant E's own reported numbers on the live record --
  ...
  scatter, t=-2.4914058108928567, n=82 training stretches.
  E (nonlin_dyn, same contact and band, reading index 0): bias -0.03452435522071541,
  z=-1.4706266338270466, n=221 training stretches.

-- is proven live, through the bridge, against the module's ported `startup_bias_d_method` /
`startup_bias_e_method` / `split_stretches_by_time` fed the identical stretches the contest's own
scripts built (bit-exact to every digit printed above); the contest's own recorded series lives
only in a gitignored scratch file and cannot be committed as a test fixture. See the decision log
for that live proof. What is pinned here, on constructed data, is that the ARITHMETIC each
function performs is the arithmetic its docstring says it performs.
"""
import numpy as np
import pytest
