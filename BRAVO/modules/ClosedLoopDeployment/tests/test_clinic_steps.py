...
def test_nonoverlapping_harmonic_window_and_misaligned_scores():
    assert CS.amplitude_response_band_mask(165.0, [13., 14., 15.], tol_hz=0.01).all()
    with pytest.raises(ValueError, match='does not match amplitude'):
        CS.within_visit_band_scores({20.: [1., 2.]}, [1.], ['a'], response_fn=lambda *a, **k: None)
