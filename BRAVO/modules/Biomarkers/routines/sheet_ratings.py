...
The heat maps pool at-home REDCap ratings. The lab's clinic and at-home testing sheets (decision
161) carry a second set of scores -- one per step, written by the clinician at the moment the
patient gave it -- that Stim Optimizer reads and the Biomarkers page never did. Measured on RCS08
on 2026-09-16 the sheets would roughly double the ratings on the four thinner contact pairs (L 0-2+
91 -> 168 on NRS). The PI's ruling: pool them, behind a switch that ships OFF, with every added
rating flagged and counted so the page says how much of a cell rests on them.
