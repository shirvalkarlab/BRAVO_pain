...
On the first live run of the two-stage path (commit 5ecd6c62) Stage 1's open-loop search froze
40 Hz at 100 us on both sides of RCS08, and the gate (``routines/stage_gate.check_rate_floor``)
then refused closed loop because 40 Hz is below the device's adaptive minimum of 55 Hz. That is the
pattern the instruction forbids: the search proposed a setting the closed-loop mode cannot be
programmed with, and only a later check caught it. The search must not propose such a setting in
the first place. This module is where the envelope is defined, so Stage 1 and the gate read ONE
definition and cannot disagree about it.

WHAT THE ENVELOPE IS, AND WHAT IT IS NOT
----------------------------------------
* ``MIN_RATE_HZ``: the adaptive minimum stimulation rate, read from
  ``percept_adaptive.MIN_ADAPTIVE_RATE_HZ`` (55 Hz, PI-stated; the A610 guide states the direction
  of the constraint but does not print the number). This is the only HARD limit the platform knows.
* The adaptive-mode CEILINGS on rate and on pulse width are NOT KNOWN. The A610 guide (p. 35) says
  both are lower than in a group without Adaptive Therapy and gives neither number, and a search of
  all 1,154 RCS08 session reports found no field carrying them
  (``ClosedLoopDeployment/device_facts.py``, the note above ``BRAINSENSE_PROGRAMMED_PAIRS``). They
  are recorded here as ``None`` and are NOT enforced; inventing a ceiling would be exactly the kind
  of guessed safety number ``percept_adaptive`` forbids.
* ``BRAINSENSE_PROGRAMMED_PAIRS`` in ``ClosedLoopDeployment/device_facts.py`` lists the (rate,
  pulse width) pairs the device has actually accepted in a BrainSense group on each side of RCS08.
  Absence from that table is NOT a prohibition (its own docstring says so), so it is read here for
  REPORTING only -- "this exact pair has / has not been demonstrated on this side" -- and never
  excludes anything. It is read lazily, through the double-import spelling this codebase uses,
  because ``ClosedLoopDeployment`` already imports ``StimOptimizer`` in several places and a
  module-level import in the other direction would risk a cycle; if it cannot be read, the
  annotation says so rather than guessing.
