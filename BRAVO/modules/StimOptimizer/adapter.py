...
This adapter uses BRAVO's ``Percept.estimateSessionDateTime`` for the session state, exactly as the
normalized Post-visit Therapy import does. That estimator cross-checks unreliable session headers
against the report's recording dates. ``GroupHistory`` keeps its own UTC ``SessionDate``. Neither
path parses the filename.
...
The Aditya adapter now uses the canonical session estimator instead of that original raw-session
choice; see ADITYA_INTEGRATION.md for the normalized-therapy comparison. Note the interaction with the
...
def exposure_epochs(stream: pd.DataFrame) -> pd.DataFrame:
    ...
    if "rate_Left" in wide and "rate_Right" in wide:
        both = wide["rate_Left"].notna() & wide["rate_Right"].notna()
        if (both & (wide["rate_Left"] != wide["rate_Right"])).any():
            raise ValueError("Bilateral frequencies differ; this optimizer's shared-frequency model is unsupported.")
    wide["freq_hz"] = freq
...
def evidence_for_participant(participant, *, hemispheres=("Left", "Right"), rates=None,
    ...
    """
    from .routines import lfp_evidence as _ev
    psd, epochs = evidence_inputs(participant, force_refresh=force_refresh, sources=sources, stream=stream)
    if psd is None:
