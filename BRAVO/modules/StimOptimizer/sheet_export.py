...
#: The lab's template workbook, local copy. Its real on-disk name has every character a filename
#: cannot carry ("[", "]", "{", "}") replaced with "_" by whatever pulled it out of Drive; the
#: logical name (what a person would call it) is
#: "[Template]RCS08 Stage 2 - {Month} 2025 Clinic Testing {MM}_{DD}_{YY}.xlsx"
#: (`titration_plan.SHEET_SOURCE`). Used only by the xlsx fallback path -- the Drive path copies
#: the template through its Drive id (`google_sheets_client.template_id`) and never touches this
#: local file.
...
def sheet_name_for(visit_date, participant_code="RCS08") -> str:
    """"RCS08 Stage 2 - September 2026 In-Clinic Testing 09_16_26" -- the lab's own visit-file
    naming convention (`titration_plan.SHEET_SOURCE`'s directory: a real month name, the four-digit
    year once, then the two-digit month_day_year the real visit files use, e.g.
