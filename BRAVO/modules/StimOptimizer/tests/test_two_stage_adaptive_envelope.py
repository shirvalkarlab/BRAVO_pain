...
def _matrix(n_per_cell=11, pw_levels=(100.0, 140.0), rates=(40.0, 165.0), seed=0, effect=1.0,
    ...
    for i, pw in enumerate(pw_levels):
        ...
        for k in range(n_per_cell):
            ...
            rows.append(row)
    d = pd.DataFrame(rows)
    # The two strata INTERLEAVED in time, as a real record's settings are (since 2026-09-17; laid
    # out one stratum after the other before that). The incumbent (the newest epoch) stays in the
    # second stratum, as before, by giving it the last date.
