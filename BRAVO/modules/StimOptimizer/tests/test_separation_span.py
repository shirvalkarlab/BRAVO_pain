...
def test_zero_slope_with_separated_arms_is_reported_without_infinite_ladder():
    out = LR.span_needed_for_separation(_res(d=.2, lo=2, hi=2.1, slope=0, a_lo=1, a_hi=2))
    assert out['verdict'] == 'slope_indistinguishable_from_zero'
    assert out['span_needed_mA'] is None
