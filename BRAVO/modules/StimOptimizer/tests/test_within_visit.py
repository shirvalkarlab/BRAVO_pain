...
def test_builder_returns_the_same_shape_build_all_does_and_screen_cells_eats_it():
    ...
    downstream -- the joint positive-pain/negative-current rule, the era-significance condition, the amplitude
    ...
    scr, selected = EV.screen_cells(ev, response_fn=LR.assess_response,
                                    pain_positive_by_channel={"ZERO_TWO_LEFT": set(CEN)})
...
@pytest.mark.parametrize('t0,win,tt,tp,error', [
    ([0], [120, 120], [60, 63], np.ones((2, 1)), 'must match'),
    ([0], [120], [60, 63], np.ones((1, 1)), 'tile_power'),
    ([0], [120], [63, 60], np.ones((2, 1)), 'sorted'),
])
def test_misaligned_settled_tiles_are_rejected(t0, win, tt, tp, error):
    with pytest.raises(ValueError, match=error):
        WV.step_settled_medians(t0, win, tt, tp)


def test_builder_audits_missing_columns_and_missing_channel_tiles():
    S = _steps(n_visits=2)
    tt, tp = _tiles(S)
    ev, aud = WV.build_within_visit_evidence(S.drop(columns=['visit']), channel='test',
        hemisphere='Left', rate_hz=55, centers_hz=CEN, tile_t=tt, tile_power=tp,
        amp_col='amp_mA_Left')
    assert ev is None and 'visit' in aud.reason_unusable
    ev, audit = WV.build_all_within_visit(S, centers_hz=CEN,
        tiles_by_channel={}, channels=['absent'], hemispheres=['Left'])
    assert not ev and not audit.usable.any()


@pytest.mark.parametrize('y,amp,vis', [
    ([1, 2], [1, 2], ['a', 'b']),
    ([1, 2, 3, 4], [1, 1, 1, 1], ['a', 'a', 'b', 'b']),
    ([0, 0, 0, 0], [1, 2, 1, 2], ['a', 'a', 'a', 'a']),
])
def test_cluster_statistic_withholds_unidentifiable_evidence(y, amp, vis):
    assert np.isnan(WV._band_t_cluster_robust(y, amp, vis, vis))


def test_permutation_rejects_misaligned_rows_and_handles_empty_sensitivity_cluster():
    power, amp, vis = _panel(effect_bands=[20.5], slope=-1)
    with pytest.raises(ValueError, match='must all match'):
        WV.band_cluster_permutation(power, amp[:-1], vis, n_perm=3)
    out = WV.band_cluster_permutation(power, amp, vis, n_perm=3, extra_thresholds=(1e20,))
