...
## 2026-09-20 (the PI: calibration): decisions 207-209
- 207: exploration only. The frozen model's slope is between-band gain pooled; within-band slopes about 1; raw fits agree with the median ratio. The PI scratched the frozen-model rewrite for a refit of the constant.
- 208: the lab's `percept-spectral-repro` benchmark cloned (commit a06afff) and run unmodified: June anchor exact (n 131, k 352.62, r 0.9927); all 583 exports through 2026-09-03: k 350.0 but r 0.823 from short 3-4 mA blocks; block gate (3 s, 6 readings) + 5-MAD raw-ratio rule: n 133, k 345.59, r 0.992. Bridge ratio as raw median 4.755 (geomean was 4.789). `routines/calibration.py`, two de-identified tables in `data/calibration/`, `.gitignore` negation. Tests RED then GREEN 9 of 9. Suites 1379 / 2 / 0 and 696 / 0.
- 209: `LSB_PER_UV2_TRANSFORM = 349.10` (midpoint), composed bridge 72.90; page labels from the served `method`/`reason` strings (`calibrationLabels.js`, jest 17 passed). First live pair confounded by a REDCap fetch (77 -> 87 reports) -- redone matched: Biomarkers response 6,698 numbers all scaled by exactly 0.9900; Closed-Loop thresholds and E1 scaled, verdicts identical; sweep built fresh on the same data: r moves at most 0.044 (ceiling exclusion in native units). Workers reloaded; chunk 802.f7e9180d.

## 2026-09-20 (the PI: "loosen the rule ... then run the refit on the median of all of the data"): decisions 210, 211
- 210: the grid's "established" = interval off zero AND beats the selection-aware shuffle bar; today R 1-3+'s best band is 0.23 [0.05, 0.41] against a bar of 0.28. The pain leg now counts "supported" (positive, interval wholly above zero). RED 4 / GREEN 16 (pytest), RED 1 / GREEN 19 (jest). Live: Stim Optimizer usable 0 -> 11 of 50, L 0-3+ Left 125 Hz selected (24.5-27.5 Hz, none on a harmonic).
- 211: `LSB_PER_UV2_TRANSFORM = 345.59` (one median over every block), bridge 72.16. Suites 1381 / 2 / 0 and 696 / 0. Matched live pairs: Biomarkers 6,698 numbers all by exactly 345.59/349.10; Closed-Loop thresholds and E1 by the factor, verdicts identical; Stim Optimizer 11 usable both; fresh sweep r moves at most 0.079. Workers reloaded; frontend rebuilt.

## 2026-09-20 (the PI: "bottom calibration plots use the latest calibration constants"): decision 212
- The dashed panel drew the frozen June log-log model (per-band gain anchors on log power, slopes 0.85 / 0.52), read by no calculation since 2026-06-28. The endpoint now serves `calibration.panel_payload` (every block with its status: 133 kept / 11 gated / 23 flagged / 3 unusable; the fit; the June reference from the same table; the bridge per centre and contact pair; the constants from `analytics`). New `CalibrationInEffectPanel.js`, raw axes; `PsdLsbPanel.js` draws `k_in_effect` beside its own fit.
- RED 7 / GREEN 7 (container), RED 1 suite / GREEN 7 (jest), 30 across the four affected jest files. Suites 1381 / 2 / 0 and 702 / 0. Live endpoint 1,860 -> 3,007 fields, the whole payload the intended change. Rebuilt (chunk 491.0552fe71); workers reloaded.
