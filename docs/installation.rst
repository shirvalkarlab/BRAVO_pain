...
You can find the Docker Images for `BRAVO Client <https://hub.docker.com/r/jcagle95/bravo-client>`_ and `BRAVO Server <https://hub.docker.com/r/jcagle95/bravo-server>`_ from Docker Hub. 
The BRAVO Client is built upon ``nginx:alpine`` package, which is a minimum image for serving static content through Port 80 (HTTP). 
The BRAVO Server is built on ``ubuntu:20.04`` image, which will serve the server through Port 3001. The Server has DEBUG enabled, and all error messages can be confirmed in the Docker Logs. 
...
The Docker Compose will create 2 volumes, one for SQL Database (SQLDatabase) and one for BRAVO data (BRAVOStorage). These 
