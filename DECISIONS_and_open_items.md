...
22. **Whether to fold the entangled routine's spectrum-and-correlation step into the newer
    ...
    recommendation, deliberately not done yet. Needs a channel-set check first (confirming both
    routines look at the same electrode contacts) and then its own field-count/difference-count
    proof on live data, the same discipline just applied to the zero-fill fix.
