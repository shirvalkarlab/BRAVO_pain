...
20. **The device route's saturation ceiling is a provisional constant.** Decision 33 requires a
    per-channel ceiling check and does not fix its value; `DEVICE_SPIKE_FOLD` in
    `three_source_response.py` excludes a sample above ten times its settled window's median. On
    RCS08 it excludes 12 samples in 11 runs and changes no settled power. The value, and whether
    a fold of the window median is the right shape of rule (a window that is mostly spikes has a
    spike for its median, so nothing is excluded), are the PI's to confirm; the tests state the
    limit. Raised 2026-09-07.
