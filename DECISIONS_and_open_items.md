...
- Every model fitted offline here holds its test rows out as blocks of TIME with the neighbouring rows dropped from training, the gap taken from the label's own decorrelation timescale, and reports each candidate plainly AND with the stimulation current taken out; an out-of-sample score is never folded, and none of this refuses anything (240).
