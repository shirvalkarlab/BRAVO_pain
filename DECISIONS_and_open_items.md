...
| 50 | **Track E: the assembled spectrum matrix and the per-recording spectrum files both move into the one store as raw kinds; the per-recording directory is deleted rather than migrated.** | His decision on 2026-09-07 from the measurements in `artifacts/adr_2026-09-07_track_e_spectrum_directories.md`: the per-recording directory was read by no page request, and the stored spectra already matched a fresh computation exactly. Closes open item 7, Track C step 1, Track D steps 1 and 2, and Track E step 2. In progress. | 2026-09-07 |
